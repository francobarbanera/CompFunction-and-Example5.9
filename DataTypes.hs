...
data GlobalType = End 
                 ...
                 | Cut -- represents a "pruned away" subterm (needed, for simplicity, when showing a pruned global type) 
...
data Process =  Inact 
              ...
              | Cutp -- represents a "pruned away" subterm (needed, for simplicity, when showing a pruned global type) 
...
instance Show Process where
     show proc = showdp 0 proc
        where
             ...
             showdp n (MkInput p [(m,proc)])  = (show p)++"?"++(show m)++"."++(showdp (n+3+(lenp p)+(lenmes m)+(if (n==0) then (lenmes m) else 5)) proc)
             showdp n (MkOutput p [(m,proc)]) = (show p)++"!"++(show m)++"."++(showdp (n+3+(lenp p)+(lenmes m)+(if (n==0) then (lenmes m) else 5)) proc)
             showdp n (MkInput p ls)  = (show p)++"?"++"{"++"\n"++(showlistp (n+(lenp p)) ls)++"}"
...
data Derivation =  Comp0 Judgment 
                 ...
                 | CompOA [Derivation] Judgment 
                 | CompIA [Derivation] Judgment 
     deriving (Eq,Show)
